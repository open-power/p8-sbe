...
// and PORE addresses are printed as hex numbers (0x...).  Strings are printed
// verbatim.
// 
...
// and PORE addresses are printed as hex numbers (0x...).  Vectors of strings
// are not supported.
// 
...
// relocatable PORE address where the input file was loaded to stdout.
//
       ...
       int i_argc, const char** i_argv)
{
    ...
    do {
        ...
        // Do the append and print the PORE address where the data was loaded.
        // We will not fail for unaligned addresses, as we have no knowledge
        // of whether or why the user wants the final PORE address.
        ...
        rc = sbe_xip_section2pore(newImage, sectionId, sectionOffset,
                                  &poreAddress);
...
TEST(void* io_image, const int i_argc, const char** i_argv)
{
    ...
    ProcSbeFixed* fixed;
    uint32_t tocSize;
    std::string longString = "A very long string";
    ...
    do {
        ...
        switch (magicKey) {
        case SBE_BASE_MAGIC:
            key = "proc_sbe_fabricinit_revision";
            rc = sbe_xip_get_string(io_image, key.c_str(), &revision);
            BOMB_IF_RC;
        ...
        if (magicKey != SBE_SEEPROM_MAGIC) {
            rc = 
	      sbe_xip_set_string(io_image, key.c_str(), longString.c_str()) ||
	      sbe_xip_get_string(io_image, key.c_str(), &revision);
            BOMB_IF_RC;
            BOMB_IF((strlen(revision) != strlen(revdup)) ||
                    (strncmp(revision, longString.c_str(), strlen(revdup)) != 0));

            rc = 
	      sbe_xip_set_string(io_image, key.c_str(), shortString.c_str()) ||
	      sbe_xip_get_string(io_image, key.c_str(), &revision);
            BOMB_IF_RC;
            BOMB_IF(strcmp(revision, shortString.c_str()) != 0);
                       ...
                       const char   **i_argv)
{
  ...
  nextSection = (void*)((uintptr_t)nextBlock + sizeSection);
  ...
  *disList = '\0'; // Make sure the buffer is NULL terminated (though probably not needed.)
  ...
  //
  rc = pore_inline_context_create(  &ctx,
                                    nextBlock,
                                    sizeSection,
                                    nextLinkOffsetBlock,
                                    0);
  if (rc)  {
    fprintf( stderr, "ERROR : %s (rc=%i)\n",pore_inline_error_strings[rc],rc);
    fprintf( stderr, "\tMore info: %s\n", DIS_ERROR_STRING(g_errorStringsDis, DIS_DISASM_ERROR));
    return SBE_XIP_DISASSEMBLER_ERROR;
  }

  while ((uintptr_t)nextBlock<(uintptr_t)nextSection) {
    ...
    // Do disassembly.
      ...
      // ...data disassembly
      if (sizeData>0)  {
       ctx.options = PORE_INLINE_LISTING_MODE | PORE_INLINE_DISASSEMBLE_DATA;
        do  {
          rc = pore_inline_disassemble( &ctx, &dis);
          ...
          if (rc)  {
            ...
            sizeDisLine = snprintf(lineDis,LISTING_STRING_SIZE,
              ...
              pore_inline_error_strings[rc],rc,sectionId);
            sizeList = sizeList + sizeDisLine;
            disList = strcat(disList,lineDis);
          }
          // Readjust list buffer size, if needed.
      ...
      // ...code disassembly
      if (sizeCode>0)  {
        ctx.options = PORE_INLINE_LISTING_MODE;
        do  {
          rc = pore_inline_disassemble( &ctx, &dis);
          ctx.options = PORE_INLINE_LISTING_MODE;
          sizeDisLine = snprintf(lineDis,LISTING_STRING_SIZE,"%s\n",dis.s);
    ...
    nextBlock = (void*)((uintptr_t)nextBlock + sizeBlock);
