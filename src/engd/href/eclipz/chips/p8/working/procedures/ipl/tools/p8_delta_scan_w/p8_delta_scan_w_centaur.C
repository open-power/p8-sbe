...
// $Id: p8_delta_scan_w_centaur.C,v 1.3 2013/02/28 13:58:54 cmolsen Exp $
//
/*------------------------------------------------------------------------------*/
...
//
/* *! USAGE : To build -                                                        */
//                       mybuildecmdprcd  p8_delta_scan_w.C
//
/* *! ASSUMPTIONS :                                                             */
...
int main( int argc, char *argv[])
{
  ...
  char			*i_fnAlterRing;
	...
	//uint64_t	i_scanSelect=0x0400080000000000; // Doesn't work
  ...
  fpCheck = fopen(i_fnMetaData, "r");
	...
	// Determine if ring type is a VPD ring and check if the supplied chiplet ID agrees
  ...
  if (!deltaRing)  {
    fprintf(stderr, "calloc() failed for deltaRing.  Stopping.");
		cleanup(initRing, alterRing, deltaRing, deltaRingRS4);
		exit(1);
  }
  ...
  if (!bufTmp)  {
	...
	//rc = write_delta_ring_to_image(	i_fnImage,
...
int write_rs4_ring_to_ref_image(char			*i_fnImage,
																...
																uint32_t	dbgl)
{
	uint32_t rc=0, rcLoc1=0, rcLoc2=0, iImg, bufLC;
	int      deltaLC=0, i;
	uint8_t  *imageByteOut;
	uint32_t fdImage, fdMetaData;
	struct stat stbuf;
	...
	uint64_t initfRS4PoreAddress=0, addrOfFwdPtr=0;
	SbeXipItem tocItem;
	...
	// Determine size of input image and memory map it.
	//
  ...
  // Allocate temporary memory to append ring layout content to .rings section.
  //
  ...
  if (rc)   {
	...
	//
  fdMetaData = open(i_fnMetaData, O_RDONLY);
	...
	// ringType = 0:  Non-VPD ring. May have meta data attached to it.
  ...
  if (i_ringType==0)  {
		...
		//
#define CENTAUR_VERSION
#ifndef CENTAUR_VERSION
	  ...
	  // Now, create the inline assembler code.
		...
		rc =      pore_MR(&ctx, A0, PC);
		rc = rc + pore_ADDS(&ctx, A0, A0, ASM_RS4_LAUNCH_BUF_SIZE);
#ifdef CENTAUR_VERSION
		rc = rc + pore_LI(&ctx, D0, uint64_t(0));
#else
		rc = rc + pore_LI(&ctx, D0, scanChipletAddress);
#endif
		rc = rc + pore_BRAD(&ctx, D0);
		if (rc || ctx.error)  {
	  ...
	  if (rc)   {
	      printf("\tWARNING: sbe_xip_find() failed: %s\n", SBE_XIP_ERROR_STRING(g_errorStrings, rc));
	      if (rc==SBE_XIP_ITEM_NOT_FOUND)  {
					printf("\tProbable cause:\n");
	  ...
	  if (rc)   {
	    printf("\tsbe_xip_append() failed: %s\n", SBE_XIP_ERROR_STRING(g_errorStrings, rc));
	    return IMGBUILD_ERR_GENERIC;
	  }
	  ...
	  printf("\tSuccessful append of RS4 ring to .initf... Next, update variable name ptr...\n");
    ...
    rc = write_vpd_ring_to_ipl_image(
		...
		if (rc)  {
		  ...
		  free(ringsBuffer);
			...
			printf("\tSuccesful update of image with VPD ring.\n");
		}
	}
	else  {
  ...
  fclose(fpImageOut);
  //system("diff -s ../ipl/sbe/bin/sbe_pnor.bin sbe_pnor_out.bin");
