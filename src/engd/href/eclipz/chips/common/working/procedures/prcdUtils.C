...
uint32_t prcdLogScomData(ecmdChipTarget &i_target, uint32_t i_addr) {
  ...
  sprintf(outstr,"(PRCD FFDC) target: %s, scom address: 0x%.8X, data: 0x%s\n", ecmdWriteTarget(i_target, ECMD_DISPLAY_TARGET_COMPRESSED).c_str(), i_addr, data.genHexLeftStr(0, data.getBitLength()).c_str());
  ecmdOutput(outstr);
...
uint32_t prcdLogScanData(ecmdChipTarget &i_target, std::string i_ringName) {
  ...
  sprintf(outstr,"(PRCD FFDC) target: %s, ring name: %s", ecmdWriteTarget(i_target, ECMD_DISPLAY_TARGET_COMPRESSED).c_str(), i_ringName.c_str());
  printed = outstr;
...
uint32_t prcdLogGPData(ecmdChipTarget &i_target, uint32_t i_gpNum) {
  ...
  sprintf(outstr,"(PRCD FFDC) target: %s, GP Register: %d, data: 0x%s\n", ecmdWriteTarget(i_target, ECMD_DISPLAY_TARGET_COMPRESSED).c_str(), i_gpNum, data.genHexLeftStr(0, data.getBitLength()).c_str());
  ecmdOutput(outstr);
...
uint32_t prcdLogFailingPart(ecmdChipTarget &i_target, std::string i_priority) {
  ...
  sprintf(outstr,"(PRCD FFDC) Chip %s has failed with priority %s\n", ecmdWriteTarget(i_target, ECMD_DISPLAY_TARGET_COMPRESSED).c_str(), i_priority.c_str());
  ecmdOutput(outstr);
